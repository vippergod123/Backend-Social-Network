...
FirestorePost.doc("PostStatus").get().then( (snapshot) => { 
...
function UpdateLastBlockAndCurrent2Firebase( last_block_height, currentBlock) { 
    FirestorePost.doc("PostStatus")
    .update({
        last_block_height: last_block_height,
        currentBlock: currentBlock,
    })
    .then(() => { 
        console.log("Update current and last post\n");
    })
    .catch((err) => {
        console.log(err);
    })
        ...
        if(block.header.num_txs === '0') {
            ...
            txs.forEach( each => {
                ...
                PushPostToFirebase(decode).then( response => { 
                    ...
                    resolve("push post to firebase success");
                }).catch( err=> { 
