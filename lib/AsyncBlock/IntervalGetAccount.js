...
FirestoreAccount.doc("AccountStatus_Test").get().then( (snapshot) => { 
...
function UpdateLastBlockAndCurrent2Firebase( last_block_height, currentBlock) { 
    FirestoreAccount.doc("AccountStatus_Test")
    .update({
                    ...
                    switch (interact) {
                        ...
                        case 2:                        
                            ...
                            if(historyLike.length > 0) {
                                historyLike.forEach(each => {
                                    var like = JSON.parse(each)
                                    if(newLike.publicKey === like.publicKey) {
                            ...
                            FirestorePost.doc(transaction.params.object).update ({
                                like: kt ? [...historyLike] : [...historyLike, JSON.stringify(newLike)],
                            }).then(() => {
