...
function  encodeCreateAccountTransaction (account, address, private_key) {
    return new Promise((resolve, reject) => { 
        var req = publicDomain + "/tx_search?query=%22account=%27"+account+"%27%22&page=2&per_page=100"   
        axios.get(req)
        .then(response => {
...
function encodeInteractTransaction  (account, hash, content, private_key) {
    return new Promise((resolve, reject) => {     
        var req = "https://komodo.forest.network/tx_search?query=%22account=%27"+account+"%27%22&&page=2&per_page=100"    
        axios.get(req)
        .then(response => {
            const data = response.data.result.txs.map((each) => {
                each.tx = decodeTransaction(each.tx);
